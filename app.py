...
# Calculate 3-month rolling average for 'Beans Sold'
df['Beans_Sold_Rolling_Avg'] = df['Beans Sold'].rolling(window=3).mean().fillna(df['Beans Sold'])

# Prepare features and target variable
X = df[['Month_Num', 'Holiday', 'Beans_Sold_Rolling_Avg']]
...
def index():
    ...
    if request.method == 'POST':
        ...
        # Retrieve the latest 3-month rolling average
        latest_3_months = df['Beans Sold'].iloc[-3:].mean()

        # Make prediction
        input_data = np.array([[month_num, is_holiday, latest_3_months]])
